...
// Use os.tmpdir() which is 100% writable in Vercel Serverless Lambdas and local machines
const TMP_DATA_FILE = path.join(os.tmpdir(), 'cowork_rooms_v2.json');
...
function loadRoomsFromDisk(): Map<string, Room> {
  ...
  try {
    ...
    console.warn('Could not read from tmpdir:', err);
  }
  return map;
}
...
function saveRoomsToDisk(map: Map<string, Room>) {
  try {
    ...
    fs.writeFileSync(TMP_DATA_FILE, JSON.stringify(obj, null, 2), 'utf-8');
  } catch (err) {
    console.warn('Could not save to tmpdir:', err);
  }
}
...
export function restoreOrUpsertRoom(room: Room): Room {
  const map = getRoomsMap();
  const normalized = room.code.trim().toUpperCase();
  
  const existing = map.get(normalized);
  if (existing) {
    // Merge tasks and members if existing
    const mergedMembers = [...existing.members];
    room.members.forEach((m) => {
      if (!mergedMembers.some((em) => em.id === m.id || em.name.toLowerCase() === m.name.toLowerCase())) {
    ...
    room.tasks.forEach((t) => {
      const existingTask = mergedTasks.find((et) => et.id === t.id);
      if (existingTask) {
        existingTask.submissions = { ...existingTask.submissions, ...t.submissions };
      } else {
        mergedTasks.push(t);
      }
    });
